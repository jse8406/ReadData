...
parser.add_argument("-y", "--year", type=str, default="2025", required=False, help="가져올 연도 (예: 2024)")
...
# 각 DB와 JSON 쌍에 대해 처리
for db_path, json_path in db_json_pairs:
    # SQLite 연결
    ...
    for month in range(1, 13):
        ...
        for start, end in date_ranges:
            ...
            if not df.empty:
                ...
                print(f"{db_path} - {year}년 {month_str}월 {start}일 ~ {end}일 데이터가 없습니다.")
        total.append(value)

    print(f"{db_path} -> {json_path}: {total}")
